...
var  newArray = strings.filter(word => word.length >= 5);
...
var str = alphabets.reduce((accumulator, currentValue) => accumulator + currentValue);
console.log(str); 
//Challenge 4: Given an array of numbers, write a code to return a first even number using find method.
const number = [1, 2, 3, 4, 5];
var firstEven = number.find(num => {if(num % 2 === 0)return num});
...
var firstSevenIndex = ranks.findIndex(rank => rank === 7);
console.log(firstSevenIndex);
...
var hasLessThan45 = marks.some(mark => mark < 45);
...
var allPositive = positive.every(num => num > 0);
...
// Delete the "phoneno" property from the object person.
delete person.phoneno;

// Using the hasOwnProperty check whether the person object has the property "bike", "house".
console.log(person.hasOwnProperty('bike')); 
console.log(person.hasOwnProperty('house'));

// Using dot and bracket notation get the output as "Mr Ajay Kumar travels by plane called Air123".
console.log("Mr " + person.name + " travels by " + Object.keys(person.vehicles)[2] + " called " + person.vehicles.plane); 

// Change the email property of person object to "ajaykumar@gmail.com".
person.email = 'ajaykumar@gmail.com';

// Generate an array of keys to the object person using object.keys() method.
